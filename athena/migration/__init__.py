"""One-way migration from Hermes (the predecessor agent) into athena v2.

The entry point lives at :mod:`athena.migration.hermes_import`. The per-domain
importers (skills, memory, sessions, config, MCP, cron, credentials) each
expose a single function that takes ``source``, ``dest``, and a
:class:`~athena.migration.report.Report` to record outcomes.
