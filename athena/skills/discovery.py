...
Two search paths: ``~/.athena/skills/`` (user-global) and
``<workspace>/.athena/skills/`` (workspace-local). Workspace wins on name
...
def search_paths(workspace: Path | None = None) -> list[Path]:
    ...
    paths = [Path.home() / ".athena" / "skills"]
    if workspace is not None:
        paths.append(workspace / ".athena" / "skills")
