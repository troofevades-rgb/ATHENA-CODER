...
#
# Usage:
#   scripts/setup.sh [--model TAG] [--no-venv] [--skip-model] [--extras "dev"]
set -euo pipefail
...
if ! "$PY" -m pip install -e "$TARGET"; then
  fail "pip install failed. On Debian/Ubuntu without --no-venv you may hit PEP 668; re-run with a venv (default) or use pipx."
  exit 1
fi
