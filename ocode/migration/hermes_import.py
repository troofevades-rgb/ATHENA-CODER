"""Entry point for ``ocode import-from-hermes``.

Composes the per-domain importers. Each importer records into the shared
:class:`~ocode.migration.report.Report`; failures inside one phase never
...
def run_import(
    ...
    *,
    ...
    """Migrate Hermes data at ``source`` into ocode v2 at ``dest``.
