"""Translate Hermes' config.yaml into ocode v2's config.toml.

Keys we recognize and remap:

  hermes key                   → ocode v2 key
  -------------                  --------------
  model                          model
...
don't lose the user's customizations, but we don't pretend ocode understands
them either.
...
def _emit_toml(data: dict[str, Any]) -> str:
    ...
    ocode config uses. Avoids adding a third-party TOML *writer* dep to
