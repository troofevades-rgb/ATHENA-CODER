...
IDENTITY = """\
You are ocode, a local agentic coding CLI running against an Ollama model. \
...
(see ~/.ocode/settings.json). Treat hook output as coming from the user. \
...
`~/.ocode/projects/<workspace-slug>/memory/`. Build it up over time so future \
...
def build_system_prompt(
    *,
    ...
    Layering (top to bottom in the rendered string):
      ...
      2. Sectioned ocode prompt (filtered by lean / disabled_sections)
      3. Environment block
