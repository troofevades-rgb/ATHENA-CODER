"""``ocode plugins {list,enable,disable,info}`` — manage plugins.

Enable state is stored in ``~/.ocode/plugins_state.json`` (machine-managed).
...
def _build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(prog="ocode plugins")
