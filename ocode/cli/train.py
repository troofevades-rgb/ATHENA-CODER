"""``ocode train {review,build-dataset,run,status}`` — closed training loop.
...
- ``run``: orchestrates LoRA → DPO (if a DPO dataset was passed or
  ...
  ``<base>-ocode-<n>`` where ``n`` is the next free index.
- ``status``: prints the last training run's metadata from
  ``~/.ocode/training_state.json``.

State file:
    ``~/.ocode/training_state.json`` holds an array of past runs.
...
def _next_output_name(base_model: str) -> str:
    ...
    n = 1
    while f"{base_tag}-ocode-{n}" in used:
        n += 1
    return f"{base_tag}-ocode-{n}"
...
def _cmd_run(args) -> int:
    ...
    print(f"  ocode model switch {output_name}   # to make it the default")
    return 0
...
def _build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(prog="ocode train")
