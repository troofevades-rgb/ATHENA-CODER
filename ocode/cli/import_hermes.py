"""``ocode import-from-hermes`` — one-shot migration of Hermes data into ocode v2."""
...
---------------------
  ...
  2. Try `ocode skill list` to confirm imported skills appear.
  3. Run `ocode` interactively; the migrated profile is loaded by default.
...
def _parse(argv: list[str]) -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        prog="ocode import-from-hermes",
        description="Migrate Hermes Agent data into ocode v2.",
    )
    ap.add_argument("--source", required=True, type=Path, help="Hermes home directory (e.g. ~/.hermes)")
    ap.add_argument("--dest", required=True, type=Path, help="ocode home directory to write into (e.g. ~/.ocode)")
