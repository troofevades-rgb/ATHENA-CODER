"""``ocode model {list,switch,info}``.

``list`` shows every Ollama model on the local box. ``switch`` updates
the user-level ``config.toml`` so subsequent ocode sessions use the new
...
def _build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(prog="ocode model")
    sub = ap.add_subparsers(dest="cmd", required=True)

    sub.add_parser("list", help="List local Ollama models.")
    p_switch = sub.add_parser(
        "switch",
        help="Set the default model for new sessions (writes ~/.ocode/config.toml).",
    )
