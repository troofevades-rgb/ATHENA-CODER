...
def test_stage_1_rollout_lists_match() -> None:
    ...
    actually_flagged = {
        t.name for t in tools.all_tools() if t.parallel_safe and not t.name.startswith("_")
    }
...
def test_confirmation_required_tools_are_never_parallel_safe() -> None:
    ...
    bad = [t.name for t in tools.all_tools() if t.requires_confirmation and t.parallel_safe]
    assert not bad, f"confirmation-required tool(s) flagged parallel_safe: {bad}"
...
_STATE_MUTATORS = {
    ...
    "TaskCreate",
    "TaskUpdate",  # mutates task store
    "clarify",  # interactive prompt
    "write_memory",
    "delete_memory",  # memory writes
    "bash_output",
    "kill_bash",  # bash process state
    "delegate_to_cli",  # external process state
    "video_generate",
    "animate_image",  # long-running write artifacts
}
...
def test_browser_session_tools_stay_serial() -> None:
    ...
    flagged = [
        n
        for n in _BROWSER_SESSION_TOOLS
...
def test_computer_screen_tools_stay_serial() -> None:
    ...
    flagged = [
        n
        for n in _COMPUTER_SCREEN_TOOLS
        if (t := tools.get_tool(n)) is not None and t.parallel_safe
    ]
...
def test_state_mutator_tools_stay_serial() -> None:
    ...
    flagged = [
        n for n in _STATE_MUTATORS if (t := tools.get_tool(n)) is not None and t.parallel_safe
    ]
    assert not flagged, f"state-mutating tools must stay serial: {flagged}"
