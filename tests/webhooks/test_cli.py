...
def test_add_with_skill_and_explicit_secret(
    isolated_profile: Path, capsys: pytest.CaptureFixture,
) -> None:
    rc = cli.main([
        "add",
...
def test_add_with_prompt_template(
    ...
    rc = cli.main([
        "add",
        "--auth", "none",
        "--prompt-template", "Summarize: {{ payload }}",
    ])
    assert rc == 0
    rows = _read_store(isolated_profile).list()
...
def test_add_with_custom_host_port_in_url(
    ...
    cli.main([
        "add", "--auth", "none", "--skill", "x",
        "--host", "athena.example.com", "--port", "8443",
    ])
    out = capsys.readouterr().out
...
def test_test_hmac_request_signs_body(
    ...
    cli.main(["add", "--auth", "hmac_sha256", "--secret", "shh",
              "--skill", "x"])
    capsys.readouterr()
    rows = _read_store(isolated_profile).list()
    rid = rows[0].id
    ...
    expected_sig = hmac.new(
        b"shh", captured["content"], hashlib.sha256,
    ).hexdigest()
...
def test_test_bearer_request_carries_authorization(
    ...
    cli.main(["add", "--auth", "bearer", "--secret", "tok-123",
              "--skill", "x"])
    capsys.readouterr()
    rows = _read_store(isolated_profile).list()
    captured: dict = {}
...
def test_test_with_custom_payload_file(
    isolated_profile: Path, capsys: pytest.CaptureFixture, tmp_path: Path,
) -> None:
    cli.main(["add", "--auth", "none", "--skill", "x"])
    capsys.readouterr()
    rows = _read_store(isolated_profile).list()
    ...
    payload_file.write_text(
        json.dumps({"custom": "value"}), encoding="utf-8",
    )
