...
def test_rejected_when_disabled(tmp_path: Path, monkeypatch):
    stub = _GitStub()
    _patch_git(monkeypatch, stub)
    monkeypatch.setattr(cli_mod, "_load_cfg", lambda: _cfg(cli_delegate_enabled=False))
...
def test_rejected_when_no_command_configured(tmp_path: Path, monkeypatch):
    stub = _GitStub()
    _patch_git(monkeypatch, stub)
    monkeypatch.setattr(cli_mod, "_load_cfg", lambda: _cfg(cli_delegate_command=None))
...
def test_captures_diff(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("athena.delegate.adapter.DelegateAdapter", _OkAdapter)
...
def test_never_auto_merges(tmp_path: Path, monkeypatch):
    ...
    class _OkAdapter:
        ...
        def run(self, task, *, cwd, timeout_s):
            return DelegateResult(status="done", exit_code=0, stdout="", stderr="", sandboxed=False)
...
def test_response_has_review_next_step(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **kw: SimpleNamespace(
            run=lambda task, cwd, timeout_s: DelegateResult("done", 0, "", "", False)
        ),
    )
...
def test_timeout_kills_delegate(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("athena.delegate.adapter.DelegateAdapter", _TimeoutAdapter)
...
def test_sandbox_used_when_enabled(tmp_path: Path, monkeypatch):
    ...
    stub = _GitStub()
    _patch_git(monkeypatch, stub)
    monkeypatch.setattr(cli_mod, "_load_cfg", lambda: _cfg(cli_delegate_sandbox=True))
    ...
    monkeypatch.setattr("athena.delegate.adapter.DelegateAdapter", _SpyAdapter)
...
def test_sandbox_not_used_when_disabled(tmp_path: Path, monkeypatch):
    ...
    stub = _GitStub()
    _patch_git(monkeypatch, stub)
    monkeypatch.setattr(cli_mod, "_load_cfg", lambda: _cfg(cli_delegate_sandbox=False))
...
def test_untrusted_output_surfaced_not_applied(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "athena.delegate.adapter.DelegateAdapter",
        lambda **kw: SimpleNamespace(
            run=lambda task, cwd, timeout_s: DelegateResult("done", 0, "ok", "", False)
        ),
    )
...
def test_worktree_prep_failure_surfaces_as_error(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("athena.delegate.adapter.DelegateAdapter", _ShouldNotRun)

    out = delegate_to_cli(task="x", repo_path=str(_make_repo(tmp_path)), base_ref="bogus")
    payload = json.loads(out)
