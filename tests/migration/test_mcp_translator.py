...
def test_copies_mcp_json_verbatim(hermes_source: Path, ocode_dest: Path, migration_report) -> None:
    data = {
        "mcpServers": {
...
def test_warns_when_no_mcp_json(hermes_source: Path, ocode_dest: Path, migration_report) -> None:
    translate_mcp(hermes_source, ocode_dest, report=migration_report)
    warnings = migration_report.entries.get("mcp_warning", [])
