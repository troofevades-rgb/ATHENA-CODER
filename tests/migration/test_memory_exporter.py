...
def test_exports_sqlite_to_markdown_files(
    ...
    _build_hermes_db(hermes_source / "memory.db", [
        {"name": "user role", "type": "user", "description": "data scientist", "body": "interested in observability"},
...
def test_rebuilds_memory_md_index(
    hermes_source: Path, ocode_dest: Path, migration_report
) -> None:
    _build_hermes_db(hermes_source / "memory.db", [
        {"name": "alpha", "body": "x"},
    ...
    export_memory(hermes_source, ocode_dest, report=migration_report)
    index = (ocode_dest / "profiles" / "default" / "memory" / "MEMORY.md").read_text(encoding="utf-8")
...
def test_preserves_created_at_and_last_used_at(
    ...
    export_memory(hermes_source, ocode_dest, report=migration_report)
    text = (ocode_dest / "profiles" / "default" / "memory" / "preserved.md").read_text(encoding="utf-8")
...
def test_warns_when_no_memory_db(
    hermes_source: Path, ocode_dest: Path, migration_report
) -> None:
    export_memory(hermes_source, ocode_dest, report=migration_report)
    warnings = migration_report.entries.get("memory_warning", [])
