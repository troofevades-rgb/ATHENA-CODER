...
- **Memory** (`memory/`, became a package in Phase 5; R2 stage 5 retired the legacy API): the canonical entry point is `athena.memory.store` (profile-keyed façade over the active `MemoryProvider`). `BuiltinFileProvider` under `memory/providers/` stores entries as `<profile_dir>/memory/<filename>.md` for the global view (what MCP server tools and `athena memory` CLI read) and `<profile_dir>/memory/legacy/<workspace-slug>/<filename>.md` for the per-workspace view (what the foreground `@tool` surface and the agent's system-prompt build read). Operators with legacy data at `~/.athena/projects/<slug>/memory/` opt into the one-shot copy via `migrate_legacy_memory = true` in config.toml — the migrator runs opportunistically on next session.
